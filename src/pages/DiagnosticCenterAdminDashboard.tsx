...
const DiagnosticCenterAdminDashboard = () => {
  ...
  useEffect(() => {
    if (dashboardData?.diagnosticCenter?._id) {
      fetchCenterReviews(dashboardData.diagnosticCenter._id);
    }
  }, [dashboardData?.diagnosticCenter?._id]);
  ...
  const fetchCenterReviews = async (centerId: string) => {
    setReviewsLoading(true);
    try {
      const response = await fetch(`/api/reviews/center/${centerId}`);
      if (response.ok) {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>What patients are saying about your center</CardDescription>
          </CardHeader>
          <CardContent>
            {reviewsLoading ? (
                ...
                {reviews.map((review) => (
                  <div key={review._id} className="border rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="ml-2 text-xs text-muted-foreground">{new Date(review.createdAt).toLocaleDateString()}</span>
                    </div>
